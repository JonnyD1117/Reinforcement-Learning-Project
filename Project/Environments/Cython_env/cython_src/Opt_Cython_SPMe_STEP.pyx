...
cpdef SPMe_step( states=None, I_input=None, full_sim=False, init_flag=False):
    """
    ...
    cdef double epsilon_sn = np.array([0.6])  # average negative active volume fraction
    cdef double epsilon_sp = np.array([0.50])  # average positive active volume fraction
    cdef double epsilon_e_n = np.array([0.3])  # Liquid [electrolyte] volume fraction (pos & neg)
    cdef double epsilon_e_p = np.array([0.3])

    cdef double F = np.array([96487])  # Faraday constant
    cdef double Rn = np.array([10e-6])  # Active particle radius (pose & neg)
    cdef double Rp = np.array([10e-6])

    cdef double R = np.array([8.314])  # Universal gas constant
    cdef double T = np.array([298.15])  # Ambient Temp. (kelvin)

    cdef double Ar_n = np.array([1])  # Current collector area (anode & cathode)
    cdef double Ar_p = np.array([1])

    cdef double Ln = np.array([100e-6])  # Electrode thickness (pos & neg)
    cdef double Lp = np.array([100e-6])
    cdef double Lsep = np.array([25e-6])  # Separator Thickness
    cdef double Lc = Ln + Lp + Lsep  # Total Cell Thickness

    cdef double Ds_n = np.array([3.9e-14])  # Solid phase diffusion coefficient (pos & neg)
    cdef double Ds_p = np.array([1e-13])
    cdef double De = np.array([2.7877e-10])  # Electrolyte Diffusion Coefficient
    cdef double De_p = De
    cdef double De_n = De

    cdef double kn = np.array([1e-5 / F])  # Rate constant of exchange current density (Reaction Rate) [Pos & neg]
    cdef double kp = np.array([3e-7 / F])

    # Stoichiometric Coef. used for "interpolating SOC value based on OCV Calcs. at 0.0069% and 0.8228%
    cdef double stoi_n0 = np.array([0.0069])  # Stoich. Coef. for Negative Electrode
    cdef double stoi_n100 = np.array([0.6760])

    cdef double stoi_p0 = np.array([0.8228])  # Stoich. Coef for Positive Electrode
    cdef double stoi_p100 = np.array([0.442])

    cdef double SOC = np.array([1])  # SOC can change from 0 to 1

    cdef double cs_max_n = np.array([(3.6e3 * 372 * 1800) / F])  # 0.035~0.870=1.0690e+03~ 2.6572e+04
    cdef double cs_max_p = np.array([(3.6e3 * 274 * 5010) / F])  # Positive electrode  maximum solid-phase concentration 0.872~0.278=  4.3182e+04~1.3767e+04

    cdef double Rf = np.array([1e-3])  #
    cdef double as_n = np.array([3 * epsilon_sn / Rn])  # Active surface area per electrode unit volume (Pos & Neg)
    cdef double as_p = np.array([3 * epsilon_sp / Rp])

    cdef double Vn = Ar_n * Ln  # Electrode volume (Pos & Neg)
    cdef double Vp = Ar_p * Lp

    cdef double t_plus = np.array([0.4])

    cdef double cep = np.array([1000])  # Electrolyte Concentration (Assumed Constant?) [Pos & Neg]
    cdef double cen = np.array([1000])

    # Common Multiplicative Factor use in SS  (Pos & Neg electrodes)
    cdef double rfa_n = np.array([1 / (F * as_n * Rn ** 5)])
    cdef double rfa_p = np.array([1 / (F * as_p * Rp ** 5)])

    cdef double epsi_sep = np.array([1])
    cdef double epsi_e = np.array([0.3])
    cdef double epsi_n = epsi_e
    cdef double gamak = np.array([(1 - t_plus) / (F * Ar_n)])

    cdef double kappa = np.array([1.1046])
    cdef double kappa_eff = np.array([kappa * (epsi_e ** 1.5)])
    cdef double kappa_eff_sep = np.array([kappa * (epsi_sep ** 1.5)])
