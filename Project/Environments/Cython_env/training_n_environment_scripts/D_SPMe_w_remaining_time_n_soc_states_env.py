from py_src.Cythonized_SPMe_w_Sensitivity_Params import SingleParticleModelElectrolyte_w_Sensitivity
import gym
...
class SPMenv(gym.Env):
    ...
    def __init__(self, time_step=1, training_duration=1800, log_dir=None, log_trial_name=None,log_data=True, SOC=.5, num_actions=10, num_states=None):
        ...
        step_size = (25.7 - -25.7) / num_actions  # Compute the Average Step-Size for "num_disc" levels
        discrete_action_values = np.arange(-25.7, 25.7, step_size)  #
        index_values = np.arange(0, num_actions, 1)
        zipped_var = zip(index_values, discrete_action_values)

        # print(f"Action Step Size: {step_size}, with {num_actions} # of discrete actions possible")
        # print(f"Discrete Action Values: {discrete_action_values}")

        # self.action_dict = {index: value*max_C_val for index, value in enumerate(discrete_action_values)}
        self.action_dict = {ind: value for ind, value in zipped_var} #{index: value*max_C_val for index, value in enumerate(discrete_action_values)}

        action_list = discrete_action_values
        self.action_space = spaces.Discrete(len(discrete_action_values))


        # print(f"ACtion Dict: {self.action_dict}")
        # print(f"Length of Action Dict {len(self.action_dict)}")
