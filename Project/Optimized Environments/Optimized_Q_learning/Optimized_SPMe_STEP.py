...
def SPMe_step(xn_old=None, xp_old=None, xe_old=None, Sepsi_p=None, Sepsi_n=None, I_input=None, init_flag=False):
    """
    ...
    if init_flag is True or xn_old is None or xp_old is None or xe_old is None or Sepsi_p is None or Sepsi_n is None:
    ...
    A_dp = np.array([[1, 1, 0], [0, 1, 1], [0, -0.003465, 0.811]])
    B_dp = np.array([[0], [0], [-1]])
    C_dp = np.array([[7.1823213e-08, 8.705844e-06, 0.0001450974]])

    A_dn = np.array([[1, 1, 0], [0, 1, 1], [0, -0.0005270265, 0.92629]])
    B_dn = np.array([[0], [0], [-1]])
    C_dn = np.array([[9.1035395451e-09, 2.82938292e-06, 0.0001209138]])

    Ae_dp = np.array([[0.964820774248931, 0], [0, 0.964820774248931]])
    Be_dp = np.array([[6.2185e-06], [6.2185e-06]])
    Ce_dp = np.array([[-39977.1776789832, 0], [0, 39977.1776789832]])

    Sepsi_A_dp = np.array([[1, 1, 0], [0, 1, 1], [0, -0.003465, 0.811]])
    Sepsi_B_dp = np.array([[0], [0], [1]])
    Sepsi_C_dp = np.array([[0.00143646294319442, 0.174116720387202, 2.90194533978671]])

    Sepsi_A_dn = np.array([[1, 1, 0], [0, 1, 1], [0, -0.0005270265, 0.92629]])
    Sepsi_B_dn = np.array([[0], [0], [1]])
...
def main():
    ...
    for num_eps in range(num_episodes):
        for t in range(1800):
            ...
            initial_step = True if t == 0 else False
